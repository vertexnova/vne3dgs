...
- **Configure and build:**
  ```bash
  cmake -B build/shared -DCMAKE_BUILD_TYPE=Debug -DVNE_GS_TESTS=ON
  cmake --build build/shared
  ```
- **Tests and examples:** `-DVNE_GS_DEV=ON`, or set `-DVNE_GS_TESTS=ON` and `-DVNE_GS_EXAMPLES=ON` separately.
