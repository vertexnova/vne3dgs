...
#==============================================================================
# Vne3dgs Documentation Generation Script
...
check_prerequisites() {
    ...
    if [[ ! -f "$PROJECT_ROOT/CMakeLists.txt" ]]; then
        log_error "Not in Vne3dgs project root. Please run from project root."
        exit 1
    fi
...
generate_api_docs() {
    ...
    cmake -S "$PROJECT_ROOT" -B "$BUILD_DIR" \
        -DENABLE_DOXYGEN=ON -DVNE_GS_LIB_TYPE=shared
    cmake --build "$BUILD_DIR" --target vne3dgs_doc_doxygen
...
# Main function
main() {
    log_info "Starting Vne3dgs documentation generation..."
...
# Help function
show_help() {
    echo "Vne3dgs Documentation Generator"
    echo ""
