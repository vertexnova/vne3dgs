...
# Autodoc:   yes
#
# This script builds Vne3dgs for macOS with Xcode integration
#==============================================================================
...
[ "$GENERATE_XCODE" = true ] && CONFIGURE_CMD="cmake -G Xcode -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DVNE_GS_LIB_TYPE=$LIB_TYPE -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_OSX_DEPLOYMENT_TARGET=10.15 -DVNE_GS_TESTS=ON $PROJECT_ROOT" \
  || CONFIGURE_CMD="cmake -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DVNE_GS_LIB_TYPE=$LIB_TYPE -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_OSX_DEPLOYMENT_TARGET=10.15 -DVNE_GS_TESTS=ON $PROJECT_ROOT"

[ "$GENERATE_XCODE" = true ] && BUILD_CMD="xcodebuild -project Vne3dgs.xcodeproj -configuration $BUILD_TYPE -parallelizeTargets -jobs $JOBS" && TEST_CMD="xcodebuild -project Vne3dgs.xcodeproj -configuration $BUILD_TYPE -target RUN_TESTS" \
...
case $ACTION in
  ...
  xcode) [ "$CLEAN_BUILD" = true ] && clean_build || ensure_build_dir; eval $CONFIGURE_CMD; echo "Xcode project: $BUILD_DIR (Vne3dgs.xcodeproj)" ;;
